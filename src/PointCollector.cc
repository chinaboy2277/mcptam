...
std::string PointCollector::Track()
{
  ...
  if(bGotNewFrame)
  {
    ...
    {
      ROS_DEBUG("PointCollector: Need to drop New MKF");
      ...
      if(!mpMap->mbGood) // map has not been initialized yet with calibration pattern points
      {
        ...
        ROS_DEBUG("PointCollector: Need to initialize map");
      ...
      else  // There is a good map
      {
        ...
        {
          ROS_DEBUG("PointCollector: Gathering calibrated trackers");
        ...
        {
          ROS_DEBUG("PointCollector: Too eager, won't add new MKF yet");
...
std::string PointCollector::Optimize()     
{
...
void PointCollector::TransferKeyFrame(MultiKeyFrame* pMKF, TrackerCalib* pCT)
{
